...
{
	int  idx = 0, channel;
	char *value = nvram_bufget(RT2561_NVRAM, "CountryCode");
	char *channel_s = nvram_bufget(RT2561_NVRAM, "Channel");
...
static int getLegacyChannel(int eid, webs_t wp, int argc, char_t **argv)
{
	char *value = nvram_bufget(RT2561_NVRAM, "AutoChannelSelect");
	...
	value = nvram_bufget(RT2561_NVRAM, "Channel");
	if (NULL == value)
		return websWrite(wp, T("9"));
	else
...
static int getLegacyWdsEncType(int eid, webs_t wp, int argc, char_t **argv)
{
	char *value = nvram_bufget(RT2561_NVRAM, "WdsEncrypType");
...
#define MBSS_INIT(field, default_value) \
	do { \
		old_value = nvram_bufget(RT2561_NVRAM, #field); \
		...
		nvram_bufset(RT2561_NVRAM, #field, new_value); \
	} while (0)
	...
	if (new_num > old_num) {
		...
		*/
		...
		for (i = old_num + 1; i <= new_num; i++) {
			nvram_bufset(RT2561_NVRAM, "WPAPSK", "12345678");
			nvram_bufset(RT2561_NVRAM, "Key1Str", "");
			nvram_bufset(RT2561_NVRAM, "Key2Str", "");
			nvram_bufset(RT2561_NVRAM, "Key3Str", "");
			nvram_bufset(RT2561_NVRAM, "Key4Str", "");
		}
		*/
	}
		...
		*/
		...
		for (i = new_num + 1; i <= old_num; i++) {
			nvram_bufset(RT2561_NVRAM, "SSID", "");
			nvram_bufset(RT2561_NVRAM, "WPAPSK", "");
			nvram_bufset(RT2561_NVRAM, "Key1Str", "");
			nvram_bufset(RT2561_NVRAM, "Key2Str", "");
			nvram_bufset(RT2561_NVRAM, "Key3Str", "");
			nvram_bufset(RT2561_NVRAM, "Key4Str", "");
...
static void legacyBasic(webs_t wp, char_t *path, char_t *query)
{
	...
	old_bssid_num = atoi(nvram_bufget(RT2561_NVRAM, "BssidNum"));
	new_bssid_num = atoi(bssid_num);

	nvram_bufset(RT2561_NVRAM, "WirelessMode", wirelessmode);
...
//#WPS
	{
		...
		if(wordlist){
			if (strcmp(wordlist, "0"))
				doSystem("iwpriv raL0 set WscConfStatus=1");
			nvram_bufset(RT2561_NVRAM, "WscConfigured", "1");
			g_wsc_configured = 1;
		}
	}
//#WPS
	...
	if (0 != strlen(mssid_2)) {
		STFs(RT2561_NVRAM, i, "SSID", mssid_2);
		i++;
	}
	if (0 != strlen(mssid_3)) {
		STFs(RT2561_NVRAM, i, "SSID", mssid_3);
		i++;
	}

	nvram_bufset(RT2561_NVRAM, "BssidNum", bssid_num);
	...
	if (new_bssid_num == 1) {
		if (!strncmp(broadcastssid, "1", 2))
			nvram_bufset(RT2561_NVRAM, "HideSSID", "0");
		else
			nvram_bufset(RT2561_NVRAM, "HideSSID", "1");
	}
	else if (new_bssid_num == 2) {
		if (!strncmp(broadcastssid, "1", 2))
			nvram_bufset(RT2561_NVRAM, "HideSSID", "0;0");
		else
			nvram_bufset(RT2561_NVRAM, "HideSSID", "1;1");
	}
	else if (new_bssid_num == 3) {
		if (!strncmp(broadcastssid, "1", 2))
			nvram_bufset(RT2561_NVRAM, "HideSSID", "0;0;0");
		else
			nvram_bufset(RT2561_NVRAM, "HideSSID", "1;1;1");
	}
	else if (new_bssid_num == 4) {
		if (!strncmp(broadcastssid, "1", 2))
			nvram_bufset(RT2561_NVRAM, "HideSSID", "0;0;0;0");
		else
			nvram_bufset(RT2561_NVRAM, "HideSSID", "1;1;1;1");
	}
	...
	if (!strncmp(sz11aChannel, "0", 2) && !strncmp(sz11bChannel, "0", 2) &&
		...
		nvram_bufset(RT2561_NVRAM, "AutoChannelSelect", "0");
	if (0 != strlen(sz11aChannel))
		nvram_bufset(RT2561_NVRAM, "Channel", sz11aChannel);
	if (0 != strlen(sz11bChannel))
		nvram_bufset(RT2561_NVRAM, "Channel", sz11bChannel);
	if (0 != strlen(sz11gChannel))
		nvram_bufset(RT2561_NVRAM, "Channel", sz11gChannel);
	...
	nvram_bufset(RT2561_NVRAM, "WdsEnable", wds_mode);
	if (strncmp(wds_mode, "0", 2)) {
		// nvram_bufset(RT2561_NVRAM, "WdsPhyMode", wds_phy_mode);
		nvram_bufset(RT2561_NVRAM, "WdsEncrypType", wds_encryp_type);
		nvram_bufset(RT2561_NVRAM, "WdsKey", wds_encryp_key);
		if (!strncmp(wds_mode, "2", 2) || !strncmp(wds_mode, "3", 2)) {
			if (0 != strlen(wds_list))
				nvram_bufset(RT2561_NVRAM, "WdsList", wds_list);
		}
	}
...
static void legacyAdvanced(webs_t wp, char_t *path, char_t *query)
{
	...
	if (NULL != nvram_bufget(RT2561_NVRAM, "BssidNum"))
		ssid_num = atoi(nvram_bufget(RT2561_NVRAM, "BssidNum"));
	else
		ssid_num = 1;
	wlan_mode = atoi(nvram_bufget(RT2561_NVRAM, "WirelessMode"));

	//set to nvram
	nvram_bufset(RT2561_NVRAM, "BGProtection", bg_protection);
	nvram_bufset(RT2561_NVRAM, "BasicRate", basic_rate);
	nvram_bufset(RT2561_NVRAM, "BeaconPeriod", beacon);
	nvram_bufset(RT2561_NVRAM, "DtimPeriod", dtim);
	nvram_bufset(RT2561_NVRAM, "FragThreshold", fragment);
	nvram_bufset(RT2561_NVRAM, "RTSThreshold", rts);
	nvram_bufset(RT2561_NVRAM, "TxPower", tx_power);
	nvram_bufset(RT2561_NVRAM, "TxPreamble", short_preamble);
	nvram_bufset(RT2561_NVRAM, "ShortSlot", short_slot);
	nvram_bufset(RT2561_NVRAM, "TxBurst", tx_burst);
	nvram_bufset(RT2561_NVRAM, "PktAggregate", pkt_aggregate);
	nvram_bufset(RT2561_NVRAM, "IEEE80211H", ieee_80211h);
	nvram_bufset(RT2561_NVRAM, "WmmCapable", wmm_capable);
	nvram_bufset(RT2561_NVRAM, "APSDCapable", apsd_capable);
	...
	*/
	...
	nvram_bufset(RT2561_NVRAM, "CountryCode", countrycode);
	if (!strncmp(countrycode, "US", 3)) {
		nvram_bufset(RT2561_NVRAM, "CountryRegion", "0");
		nvram_bufset(RT2561_NVRAM, "CountryRegionABand", "0");
	}
	else if (!strncmp(countrycode, "JP", 3)) {
		nvram_bufset(RT2561_NVRAM, "CountryRegion", "1");
		nvram_bufset(RT2561_NVRAM, "CountryRegionABand", "9");
	}
	else if (!strncmp(countrycode, "FR", 3)) {
		nvram_bufset(RT2561_NVRAM, "CountryRegion", "1");
		nvram_bufset(RT2561_NVRAM, "CountryRegionABand", "2");
	}
	else if (!strncmp(countrycode, "TW", 3)) {
		nvram_bufset(RT2561_NVRAM, "CountryRegion", "0");
		nvram_bufset(RT2561_NVRAM, "CountryRegionABand", "3");
	}
	else if (!strncmp(countrycode, "IE", 3)) {
		nvram_bufset(RT2561_NVRAM, "CountryRegion", "1");
		nvram_bufset(RT2561_NVRAM, "CountryRegionABand", "1");
	}
		...
		nvram_bufset(RT2561_NVRAM, "CountryCode", "");
	}
...
static void legacyWmm(webs_t wp, char_t *path, char_t *query)
{
	...
	if (0 != strlen(ap_aifsn_all))
		nvram_bufset(RT2561_NVRAM, "APAifsn", ap_aifsn_all);
	if (0 != strlen(ap_cwmin_all))
		nvram_bufset(RT2561_NVRAM, "APCwmin", ap_cwmin_all);
	if (0 != strlen(ap_cwmax_all))
		nvram_bufset(RT2561_NVRAM, "APCwmax", ap_cwmax_all);
	if (0 != strlen(ap_txop_all))
		nvram_bufset(RT2561_NVRAM, "APTxop", ap_txop_all);
	if (0 != strlen(ap_acm_all))
		nvram_bufset(RT2561_NVRAM, "APACM", ap_acm_all);
	if (0 != strlen(ap_ackpolicy_all))
		nvram_bufset(RT2561_NVRAM, "AckPolicy", ap_ackpolicy_all);
	if (0 != strlen(sta_aifsn_all))
		nvram_bufset(RT2561_NVRAM, "BSSAifsn", sta_aifsn_all);
	if (0 != strlen(sta_cwmin_all))
		nvram_bufset(RT2561_NVRAM, "BSSCwmin", sta_cwmin_all);
	if (0 != strlen(sta_cwmax_all))
		nvram_bufset(RT2561_NVRAM, "BSSCwmax", sta_cwmax_all);
	if (0 != strlen(sta_txop_all))
		nvram_bufset(RT2561_NVRAM, "BSSTxop", sta_txop_all);
	if (0 != strlen(sta_acm_all))
		nvram_bufset(RT2561_NVRAM, "BSSACM", sta_acm_all);

	nvram_commit(RT2561_NVRAM);

	doSystem("ralink_init make_wireless_config 2561");
	if (0 == getIfLive("rai0")) {
...
static void legacywGetSecurity(webs_t wp, char_t *path, char_t *query)
{
	getSecurity(RT2561_NVRAM, wp, path, query);		//in wireless.c
}

static void LEGACYSecurity(webs_t wp, char_t *path, char_t *query)
{
	Security(RT2561_NVRAM, wp, path, query);		// in wireless.c
}
