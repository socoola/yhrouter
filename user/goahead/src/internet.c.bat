...
static int get3gSignal(int eid, webs_t wp, int argc, char_t **argv)
{
 ...
 //   stream = popen( "comgt -d /dev/ttyUSB2 -s /etc_ro/ppp/3g/signal.scr", "r" ); //将“ls －l”命令的输出 通过管道读取（“r”参数）到FILE* stream
	...
	{
	//	stream=popen("3GInfo -d /dev/ttyUSB2 -s","r");
		stream=popen("comgt -d /dev/ttyUSB2 -s /etc_ro/ppp/3g/signal.scr","r");
	}
...
static int get3gAttachState(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if(strcmp(pSubDev, "HUAWEI-EM660") == 0)
	{
		switch(atoi(nettype))
		{
			case 0:
...
int selectOperator(char_t *pName)
{
    int i=0;
    char rec[128];
    char Operator_name[32], nettype[8], dialnum[32], initialcmd[128], user[32], password[32];
    int netype_int;
...
static void setWan(webs_t wp, char_t *path, char_t *query)
{
	...
	if (!strncmp(ctype, "STATIC", 7) || !strcmp(opmode, "0")) {
        ...
        char_t *g3dev, *pin, *g3opmode, *g3idletime, *th, *tm, *tpsel;
...
{
	char rule[8192];
    char *Operator_name, *nettype, *dialnum, *initialcmd, *user, *password, *authtype, *compress;
    char *operators;
	...
	if(( operators = nvram_bufget(RT2860_NVRAM, "Operators")) && strlen( operators) )
		snprintf(rule, sizeof(rule), "%s|%s\\%d\\%s\\%s\\%s\\%s\\%d\\%d",  operators, 
		    Operator_name, nettype_int, dialnum, initialcmd, user, password, authtype_int, bcompress);
	else
		snprintf(rule, sizeof(rule), "%s\\%d\\%s\\%s\\%s\\%s\\%d\\%d",
		    Operator_name, nettype_int, dialnum, initialcmd, user, password, authtype_int, bcompress);
...
static int showOperatorsASP(int eid, webs_t wp, int argc, char_t **argv)
{
	int i=0;
	char rec[128];
    char Operator_name[32], nettype[8], dialnum[32], initialcmd[128], user[32], password[32], authtype[8], compress[8];
	...
	while(getNthValueSafe(i++, rules, '|', rec, sizeof(rec)) != -1 ){
        ...
        if((getNthValueSafe(6, rec, '\\', authtype, sizeof(authtype)) == -1)){
			continue;
		}

        authtype_int = atoi(authtype);

        if((getNthValueSafe(7, rec, '\\', compress, sizeof(compress)) == -1)){
			continue;
		}
		...
		websWrite(wp, T("<tr>\n"));
		// output No.
		websWrite(wp, T("<td> <input type=\"radio\" name=\"RuleNo\" value=\"%s\" onclick=\"MSPListOnSelect(%d)\"> </td>"), 
		Operator_name, i-1);
