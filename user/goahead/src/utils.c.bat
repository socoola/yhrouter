...
static int getCfgGeneral(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (1 == type) {
		...
		if(strstr(field,"g3_initial_cmd"))
		{
			return websWrite(wp, T(""));
		}
		else
...
static int getSysUptime(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if(strlen(buf)<10)	return	websWrite(wp,T("&nbsp;"));


		if(strstr(buf,"day"))
		{
			p=strchr(buf,'p');	
			if(p==NULL)	return	websWrite(wp,T("&nbsp;"));
			p++;
			p1=strchr(p,',');
			if(p1==NULL)	return	websWrite(wp,T("&nbsp;"));
			p1++;
			p1=strchr(p1,',');
			if(p1==NULL)	return	websWrite(wp,T("&nbsp;"));
			strncpy(buf1,p,p1-p);
